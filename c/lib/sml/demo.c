...
int main() {
  ...
  printf("  /* The \"cumulative\" functions reveal the layout (by-row or "
         "by-column)*/\n");
  /* The "cumulative" functions reveal the layout (by-row or by-column)*/
  printf("  /* If a matrix consists of 1 row or 1 column, layout makes no "
         "difference */\n");
  ...
  printf("  MatWMul (\" wt\", Y, V, S, U);   /*  Y <- ginv(X) = V * diag(1/S) "
         "* S' */\n");
