...
/************************************************************/
REAL
sml_hypot(REAL a, REAL b) // sqrt(a*a + b*b)
/*
...
/*----------------------------------------------*/
int
MatIsSquare(MATRIX a)
{
  return (MatRows(a) == MatCols(a));
}
int
Matmini(int a, int b)
{
...
size_t
Matminu(size_t a, size_t b)
{
  return ((a) < (b) ? (a) : (b));
}
size_t
Matmaxu(size_t a, size_t b)
{
  return ((a) > (b) ? (a) : (b));
}
REAL
Matminr(REAL a, REAL b)
{
  return ((a) < (b) ? (a) : (b));
}
REAL
Matmaxr(REAL a, REAL b)
{
  return ((a) > (b) ? (a) : (b));
}
double
Matmind(double a, double b)
{
  return ((a) < (b) ? (a) : (b));
}
double
Matmaxd(double a, double b)
{
  return ((a) > (b) ? (a) : (b));
}
/*----------------------------------------------*/
void
MatI(MATRIX a, size_t n, REAL c) /* a = diag(c), n by n */
{
  MatReDim(a, n, n);
  if (MatErr())
    return;
...
/*----------------------------------------------*/
void
MatJ(MATRIX a, size_t m, size_t n, REAL c) /* a = m by n, filled with c */
{
  MatReDim(a, m, n);
  if (MatErr())
    return;
...
/* returns 1 if square and symmetric, 0 otherwise */
{
  ...
  for (i = 0; i != m; i++)
    for (j = 0; j != i; j++)
      if (MatGet0(a, i, j) != MatGet0(a, j, i))
        return 0;
...
/*----------------------------------------------*/
void
MatFillD(MATRIX a, REAL x) /*       diag(a) = x */
{
  size_t i, n;
...
void
MatBFill(MATRIX a, size_t i1, size_t i2, size_t j1, size_t j2, REAL x)
/* a[i1:i2, j1:j2] <- x */
/* fill block rows i1-i2, cols j1-j2 of a with x */
{
  size_t i, j, error;

  error = (i1 > i2) || (j1 > j2) || (i1 < 1) || (i2 > MatRows(a)) || (j1 < 1) ||
          (j2 > MatCols(a));
...
void
MatFillCol(MATRIX a, size_t j, REAL x)
...
void
MatFillRow(MATRIX a, size_t i, REAL x)
...
void
MatSetDiag(MATRIX a, MATRIX c) /* diag(a) <- c */
...
void
MatGetDiag(MATRIX a, MATRIX c) /* a <- diag(c) */
...
*/
{
  ...
  error = (i1 > i2) || (j1 > j2) || (i1 < 1) || (i2 > MatRows(b)) || (j1 < 1) ||
          (j2 > MatCols(b)) || (ii1 < 1) || (ii1 > MatRows(a)) || (jj1 < 1) ||
          (jj1 > MatCols(a));
...
void
MatAddDiag(MATRIX a, MATRIX b, MATRIX c) /* a <- b + diag(c) */
...
static void
MatMul2(MATRIX a, MATRIX b, MATRIX c) /* a = b * c */
...
void
MatMul(MATRIX a, MATRIX b, MATRIX c) /* a = b * c */
...
static void
MatPreLower2(MATRIX a, MATRIX b, MATRIX c) /* a = b * c */
/* assumes a<>b, a<>c and cols(b)=rows(c) */
/* assumes valid dimensions of a, b, c */
...
void
MatPreLower(MATRIX a, MATRIX b, MATRIX c) /* a = b * c */
...
static void
MatPreDiag2(MATRIX a, MATRIX b, MATRIX c)
/* a = diag(b) * c */
/* assumes valid dims */
{
...
void
MatPreDiag(MATRIX a, MATRIX b, MATRIX c) /* a <- diag(b) * c */
...
static void
MatPostDiag2(MATRIX a, MATRIX b, MATRIX c)
/* a = b * diag(c) */
/* assumes valid dims */
{
...
void
MatPostDiag(MATRIX a, MATRIX b, MATRIX c) /* a <- b * diag(c) */
/* c must be a column vector */
{
  MATRIX tmp;
...
/*----------------------------------------------*/
void
MatTran(MATRIX xt, MATRIX x) /* xt = x'  */
                             /* xt == x allowed */
{
...
/*----------------------------------------------*/
static REAL2
MatRowxRow1(MATRIX x, size_t i, MATRIX y, size_t j, MATRIX w, int use_w)
/*
...
/*----------------------------------------------*/
REAL2
MatRowxRow(MATRIX x, size_t i, MATRIX y, size_t j, MATRIX w, int use_w)
/*
x[i,] * y[j,]` if use_w==0,
...
*/
{
  ...
  err = (MatCols(y) != xcols);
  if (use_w)
    err = (err || MatRows(w) != xcols || MatCols(w) != 1);
...
/*----------------------------------------------*/
static REAL2
MatColxCol1(MATRIX x, size_t i, MATRIX y, size_t j, MATRIX w, int use_w)
/*
...
/*----------------------------------------------*/
REAL2
MatColxCol(MATRIX x, size_t i, MATRIX y, size_t j, MATRIX w, int use_w)
/*
...
*/
{
  ...
  err = (MatRows(y) != xrows);
  if (use_w)
    err = (err || MatRows(w) != xrows || MatCols(w) != 1);
  if (err) {
...
/*----------------------------------------------*/
static REAL2
MatRowxCol1(MATRIX x, size_t i, MATRIX y, size_t j, MATRIX w, int use_w)
/*
   returns inner product of i-th row of x
...
/*----------------------------------------------*/
REAL2
MatRowxCol(MATRIX x, size_t i, MATRIX y, size_t j, MATRIX w, int use_w)
/*
x[i,] * y[,j]  if use_w==0,
x[i,] * diag(w) * y[,j]  if use_w==1.
if use_w==1, w should be a column vector
...
static void
MatXtWY1(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
...
static void
MatXtWY(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
/* if use_w==0  :   a = x' * y
 ...
 w must be a column vector */
{
  ...
  ok = (xrows == MatRows(y));
  if (use_w)
    ok = ok && (xrows == MatRows(w) && MatCols(w) == 1);
...
static void
MatXWYt1(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
/*  assumes  dims correct,  a != x, a != y */
/*  handles  x == y efficiently  */
{
...
static void
MatXWYt(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
/* if use_w==0  :   a = x * y'
   if use_w==1  :   a = x * diag(w) * y'
 w must be a column vector */
{
  ...
  ok = (xcols == ycols);
  if (use_w)
    ok = ok && (xcols == MatRows(w) && MatCols(w) == 1);
...
static void
MatXWY1(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
/*  assumes  dims correct,  a != x, a != y */
{
  int xrows, ycols;
  int i, j;
...
static void
MatXWY(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
/* if use_w==0  :   a = x * y
   if use_w==1  :   a = x * diag(w) * y
 w must be a column vector */
{
...
static void
MatXtWYt(MATRIX a, MATRIX x, MATRIX w, MATRIX y, int use_w)
/* if use_w==0  :   a = x' * y'
   if use_w==1  :   a = x' * diag(w) * y'
 w must be a column vector */
{
...
void
MatWMul(const char* op, MATRIX a, MATRIX b, MATRIX w, MATRIX c)
/*
...
/*----------------------------------------------*/
REAL2
MatCol1normalize(MATRIX a, size_t j)
/*
...
/*----------------------------------------------*/
REAL2
MatCol2normalize(MATRIX a, size_t j)
/*
...
/*----------------------------------------------*/
REAL2
MatColinormalize(MATRIX a, size_t j)
/*
...
/*----------------------------------------------*/
int
Matdaxpy_c(MATRIX z, size_t i, REAL a, MATRIX x, size_t j, MATRIX y, size_t k)
/*
z[,i] <- a x[,j] + y[,k]
...
*/
{
  ...
  if (i < 1 || i > MatCols(z) || j < 1 || j > MatCols(x) || k < 1 ||
      k > MatCols(y)) {
...
/*----------------------------------------------*/
int
MatCompanion(MATRIX comp, MATRIX a)
/*
...
REAL
MatMin(MATRIX a) /*  min element of a */
{
  size_t i, j, m = MatRows(a), n = MatCols(a);
  REAL s;
...
REAL
MatMax(MATRIX a) /*  max element of a */
{
  size_t i, j, m = MatRows(a), n = MatCols(a);
  REAL s;
...
REAL
MatMaxAbs(MATRIX a) /*  max |element| of a */
{
  size_t i, j, m = MatRows(a), n = MatCols(a);
  REAL s;
...
REAL
MatMinAbs(MATRIX a) /*  min |element| of a */
{
  size_t i, j, m = MatRows(a), n = MatCols(a);
  REAL s;
...
REAL2
MatSum(MATRIX a) /*  sum of elements of a */
{
  size_t i, j, m = MatRows(a), n = MatCols(a);
...
REAL2
MatSumAbs(MATRIX a) /*  sum of |elements| of a */
{
  size_t i, j, m = MatRows(a), n = MatCols(a);
...
REAL2
MatSS(MATRIX a) /*  sum of squares of a */
{
  REAL2 result;
  MATRIX r = MatDim(2, 1);
...
void
MatExpE(MATRIX a, MATRIX b) /* a = exp(b) elementwise  */
{
...
void
MatLogE(MATRIX a, MATRIX b) /* a = log(b) elementwise  */
{
  size_t i, j, rows = MatRows(b), cols = MatCols(b);
...
void
MatSqrtE(MATRIX a, MATRIX b) /* a = b^0.5 elementwise  */
...
void
MatRecipE(MATRIX a, MATRIX b) /* a = 1/b elementwise  */
/* 1/0 gives inf  */
...
void
MatRecipE0(MATRIX a, MATRIX b) /* a = 1/b elementwise  */
/* anything/0 := 0 */
...
void
MatApplyE(MATRIX a, REAL (*f)(REAL), MATRIX b)
...
void
MatMulE(MATRIX a, MATRIX b, MATRIX c) /* a = b * c elementwise */
{
  size_t i, j, rows = MatRows(b), cols = MatCols(b);
...
/*----------------------------------------------*/
void
MatDivE(MATRIX a, MATRIX b, MATRIX c) /* a = b / c   el-wise */
{
  size_t i, j, rows = MatRows(b), cols = MatCols(b);
...
/*----------------------------------------------*/
void
MatDivE0(MATRIX a, MATRIX b, MATRIX c) /* a = b / c   el-wise */
{
  size_t i, j, rows = MatRows(b), cols = MatCols(b);
...
static void
MatAddPE(MATRIX a, MATRIX b) /* a = a + b */
/*
Called when: a != b
...
static void
MatAdd2(MATRIX a, MATRIX b, MATRIX c) /* a = b + c */
/*
Called when: a != b && a != c
Assumes conformability
...
void
MatAdd(MATRIX a, MATRIX b, MATRIX c) /* a = b + c */
...
static void
MatSubME(MATRIX a, MATRIX b) /* a = a - b */
/*
Called when: a != b
Assumes conformability
*/
...
static void
MatSub2(MATRIX a, MATRIX b, MATRIX c) /* a = b - c */
/*
Called when: a != b && b != c
Assumes conformability
...
void
MatSub(MATRIX a, MATRIX b, MATRIX c) /* a = b - c */
...
void
MatAddScalar(MATRIX a, MATRIX b, REAL c) /* a = b + c */
{
  size_t i, j;
...
void
MatMulScalar(MATRIX a, MATRIX b, REAL c) /* a = b * c */
...
*/
{
  ...
  for (; h > 0; h /= 3) {
    ...
    for (i = h; i < r; ++i) {
      size_t j = i;
      REAL v = MatGet0(A, i, 0);
      while (j >= h && v < MatGet0(A, j - h, 0)) {
        MatSet0(A, j, 0, MatGet0(A, j - h, 0));
        j -= h;
      }
      MatSet0(A, j, 0, v);
    }
